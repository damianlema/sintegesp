...
class sqlImport {
	...
	//retrieving the vars
	function sqlImport ($host, $user,$pass, $ArchivoSql, $anioCambiar) {
	$this -> host = $host;
	$this -> user = $user;
	$this -> pass = $pass;
	$this -> ArchivoSql = $ArchivoSql;
	$this -> anioCambiar = $anioCambiar;
	$this->dbConnect();
	}
	...
	function dbConnect () {
		...
		$this->sql = mysql_query("create database if not exists ".$this->anioCambiar."", $this->con)or die(mysql_error());
		$this->db = mysql_select_db($this -> anioCambiar, $this -> con);
	}
		...
		//if we're connected to DB
   		if ($this -> con !== false) 
   		{
			...
			foreach ($sqlArray as $stmt){
				...
				//$sqlArrayToExecute[] = $stmt;
				// making the query
				$result = mysql_query(utf8_decode($stmt),$this->con);
				if (!$result)
				{
