...
var hora = new Date();
	if(hora.getHours() == 13 || hora.getHours() == 0 || hora.getHours() == 8 || hora.getHours() == 10 || hora.getHours() == 12 hora.getHours() == 15 || hora.getHours() == 17 || hora.getHours() == 19){
		window.location.href='recalcular_presupuesto_script.php';
	}	
}
</script>
<body onLoad="setInterval('verificarFecha()',3600000);">
<?
$i=1;
echo "BIENVENIDO AL SCRIPT<br /><br />";
$sql_maestro_presupuesto = mysql_query("select * from maestro_presupuesto")or die("ERROR SELECCIONAR MAESTRO: ".mysql_error());
while($bus_maestro_presupuesto = mysql_fetch_array($sql_maestro_presupuesto)){
$sql_clasificador_presupuestario = mysql_query("select * from clasificador_presupuestario where idclasificador_presupuestario = '".$bus_maestro_presupuesto["idclasificador_presupuestario"]."'");
$bus_clasificador_presupuestario =  mysql_fetch_array($sql_clasificador_presupuestario);
echo "<strong>".$i."&nbsp;-&nbsp;</strong>".$bus_clasificador_presupuestario["denominacion"]."<br>";	
$i++;
...
$sql_maestro_presupuesto = mysql_query("select * from maestro_presupuesto, clasificador_presupuestario 
									   		where clasificador_presupuestario.idclasificador_presupuestario = maestro_presupuesto.idclasificador_presupuestario
												AND clasificador_presupuestario.sub_especifica<>'00'
												")or die("ERROR SELECCIONAR MAESTRO: ".mysql_error());
	...
	$sql_suma_orden_compra = mysql_query("select SUM(partidas_orden_compra_servicio.monto) as total_suma,
		...
		//echo " antes de compra ".$bus_maestroE["total_compromisos"];				
				...
				if($num_consultar_rendicion > 0){
					...
					$total_suma_compromisos = $bus_rendicion["total_compromisos"];
					$total_suma_pago_procesado = $bus_rendicion["total_causados"];
					$total_suma_pago_pagado = $bus_rendicion["total_pagados"];
					$total_aumento = $bus_rendicion["total_aumento"];
					$total_disminuciones = $bus_rendicion["total_disminuciones"];
				...
				$sql_update_especifica = mysql_query("update maestro_presupuesto
															set 
															total_compromisos = total_compromisos +'".$total_suma_compromisos."',
															total_disminucion = total_disminucion +'".$total_disminuciones."',
