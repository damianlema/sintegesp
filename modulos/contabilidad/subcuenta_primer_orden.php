<script src="modulos/contabilidad/js/subcuenta_primer_orden_ajax.js"></script>

    <br>
<h4 align=center>
Sub Cuenta de Primer Orden
</h4>
<h2 class="sqlmVersion"></h2>
<br>

<center>
<a href="#" onClick="document.getElementById('divTipoOrden').style.display='block'; pdf.location.href='lib/<?=$_SESSION["rutaReportes"]?>/contabilidad.php?nombre=contabilidad_subcuenta1';">
...
<input name="idsubcuenta_primer" type="hidden" id="idsubcuenta_primer"/>
    ...
    <td><label>
      ...
      $sql_grupo = mysql_query("select grupo_cuentas_contables.codigo as codigo_grupo,
										...
										cuenta_cuentas_contables.idcuenta_cuentas_contables as idcuenta
	  										FROM 
										grupo_cuentas_contables, subgrupo_cuentas_contables, rubro_cuentas_contables, cuenta_cuentas_contables
											WHERE 
	  ...
	  while($bus_grupo = mysql_fetch_array($sql_grupo)){
	  	?>
		<option value="<?=$bus_grupo["idcuenta"]?>">(<?=$bus_grupo["codigo_grupo"]?>.<?=$bus_grupo["codigo_subgrupo"]?>.<?=$bus_grupo["codigo_rubro"]?>.<?=$bus_grupo["codigo_cuenta"]?>) <?=$bus_grupo["denominacion_cuenta"]?></option>
		<?
	  }
	  ?>
      </select>
...
<td>
    ...
    $sql_grupo = mysql_query("select grupo_cuentas_contables.codigo as codigo_grupo,
										...
										ORDER BY codigo_grupo, codigo_subgrupo, codigo_rubro, codigo_cuenta")or die(mysql_error());
	  ?>
      <select name="cuentaBuscar" id="cuentaBuscar">
      <option value="0">.:: Seleccione ::.</option>
	  <?
	  while($bus_grupo = mysql_fetch_array($sql_grupo)){
	  	?>
