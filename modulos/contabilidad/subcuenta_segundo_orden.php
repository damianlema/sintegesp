<script src="modulos/contabilidad/js/subcuenta_segundo_orden_ajax.js"></script>

    <br>
<h4 align=center>
Sub Cuenta de Segundo Orden
</h4>
<h2 class="sqlmVersion"></h2>
<br>

<center>
<a href="#" onClick="document.getElementById('divTipoOrden').style.display='block'; pdf.location.href='lib/<?=$_SESSION["rutaReportes"]?>/contabilidad.php?nombre=contabilidad_subcuenta2';">
...
<input name="idsubcuenta_segundo" type="hidden" id="idsubcuenta_segundo"/>
<table width="30%" border="0" align="center" cellpadding="4" cellspacing="0">
  <tr>
    <td align="right" class='viewPropTitle'>Subcuenta de Primer Nivel</td>
    <td><label>
      <?
      $sql_grupo = mysql_query("select grupo_cuentas_contables.codigo as codigo_grupo,
										...
										order by codigo_grupo, codigo_subgrupo, codigo_rubro, codigo_cuenta, codigo_subcuenta_primer")or die(mysql_error());
	  ?>
      <select name="idsubcuenta_primer" id="idsubcuenta_primer">
      <option value="0">.:: Seleccione ::.</option>
	  <?
	  while($bus_grupo = mysql_fetch_array($sql_grupo)){
	  	?>
		<option value="<?=$bus_grupo["idsubcuenta_primer"]?>">(<?=$bus_grupo["codigo_grupo"]?>.<?=$bus_grupo["codigo_subgrupo"]?>.<?=$bus_grupo["codigo_rubro"]?>.<?=$bus_grupo["codigo_cuenta"]?>.<?=$bus_grupo["codigo_subcuenta_primer"]?>) <?=$bus_grupo["denominacion_subcuenta_primer"]?></option>
		<?
	  }
	  ?>
      </select>
  ...
  <tr>
    ...
    <td><label>
      <input name="codigo" type="text" id="codigo" size="2" maxlength="2">
    </label></td>
  </tr>
  <tr>
    <td align="right" class='viewPropTitle'>Denominacion</td>
    <td><label>
      <input name="denominacion" type="text" id="denominacion" size="150">
    </label></td>
  </tr>
  <tr>
    <td>&nbsp;</td>
...
<td>
	 <?
      $sql_grupo = mysql_query("select grupo_cuentas_contables.codigo as codigo_grupo,
										...
										ORDER BY codigo_grupo, codigo_subgrupo, codigo_rubro, codigo_cuenta, codigo_subcuenta_primer")or die(mysql_error());
	  ?>
      <select name="subcuenta_primerBuscar" id="subcuenta_primerBuscar">
      <option value="0">.:: Seleccione ::.</option>
	  <?
	  while($bus_grupo = mysql_fetch_array($sql_grupo)){
	  	?>
