...
if ($_GET["accion"]==111 || $_GET["accion"] == 112){   // si entra para modificar o eliminar busca el registro para cargar los datos en el formulario
...
<body>
	...
	<br>
<form id="frmsnc_detalle_grupo" action="principal.php?modulo=3&accion=<?=$_GET["accion"]?>" method="POST" enctype="multipart/form-data">	
...
<tr>
			  <td align='right' class='viewPropTitle'>Clases:</td>
			  <td class=''>
              <?php
              $sql = mysql_query("select * from snc_grupo_actividad order by codigo");
			  ?>
              <select name="grupo_actividad" id="grupo_actividad" <? if($_GET["accion"] == 111){echo " disabled";}?>>
              <?php
              ...
              &nbsp; <a href='principal.php?modulo=3&accion=82' class='Browse'><img src="imagenes/nuevo.png" border="0" title="Nuevo SNC Detalle de Grupo"></a>
          	...
          	<tr>
                ...
                <td>
                	<select name="ordenarPor" id="ordenarPor">
                    	 <option value="snc_grupo_actividad.descripcion">Clases</option>
                         <option value="snc_detalle_grupo.descripcion">Productos</option>
                         <option value="snc_detalle_grupo.codigo">C&oacute;digo</option>
                    </select>               
                </td>
            </tr>
            <tr>
          ...
          <tr>
				...
				<td class=''><input type="text" name="codigo" maxlength="4" size="9" id="codigo" <?php echo 'value="'.$regsnc_detalle_grupo['codigo'].'"';?> <? if($_GET["accion"] == 111){echo " disabled";}?> onKeyUp="validarVacios('codigo', this.value, 'frmsnc_detalle_grupo')" onBlur="validarVacios('codigo', this.value, 'frmsnc_detalle_grupo')" autocomplete="OFF" style="padding:0px 20px 0px 0px;"></td>
			</tr>
			<tr>
				<td align='right' class='viewPropTitle'>Descripci&oacute;n:</td>
				<td class=''><input type="text" name="descripcion" maxlength="255" size="130" id="descripcion" <?php echo 'value="'.$regsnc_detalle_grupo['descripcion'].'"';?> onKeyUp="validarVacios('descripcion', this.value, 'frmsnc_detalle_grupo')" onBlur="validarVacios('descripcion', this.value, 'frmsnc_detalle_grupo')" autocomplete="OFF" style="padding:0px 20px 0px 0px;"></td>
			</tr>
...
<table align=center cellpadding=2 cellspacing=0>
	...
	echo "<input type='hidden' value='true' name='pop' id='pop'>";
					...
					if($_GET["accion"] != 111 and $_GET["accion"] != 112 and in_array(110, $privilegios) == true){
						echo "<input align=center class='button' name='ingresar' type='submit' value='Ingresar'>";
					}
				
					if($_GET["accion"] == 111 and in_array($_GET["accion"], $privilegios) == true){
						echo "<input align=center class='button' name='modificar' type='submit' value='Modificar'>";
					}
				
					if($_GET["accion"] == 112 and in_array($_GET["accion"], $privilegios) == true){
					...
					<td align="center">
					  ...
					  <thead>
								<tr>
                                    ...
                                    <td align="center" class="Browse">Producto</td>
						  ...
						  <tr bgcolor='#e7dfce' onMouseOver="setRowColor(this, 0, 'over', '#e7dfce', '#EAFFEA', '#FFFFAA')" onMouseOut="setRowColor(this, 0, 'out', '#e7dfce', '#EAFFEA', '#FFFFAA')">
								...
								$bus = mysql_fetch_array($sql);
									...
									if(in_array(111, $privilegios) == true){
										echo "<td align='center' class='Browse' width='3%'><a href='principal.php?modulo=3&accion=111&c=$c&grupo_actividad=".$bus["idsnc_grupo_actividad"]."' class='Browse'><img src='imagenes/modificar.png' border='0' alt='Modificar' title='Modificar'></a></td>";
									}
									if(in_array(112, $privilegios)){
										echo "<td align='center' class='Browse' width='3%'><a href='principal.php?modulo=3&accion=112&c=$c' class='Browse'><img src='imagenes/delete.png' border='0' alt='Borrar' title='Borrar'></a></td>";	
									}
									echo "</tr>";
									}
							}?>
...
if($_GET["accion"] == 110 and in_array(110, $privilegios) == true){
	if (mysql_num_rows($busca_existe_registro)>0){
		mensaje("Disculpe ya este registro existe");
		redirecciona("principal.php?modulo=3&accion=82");
	}else{
	...
	if ($_GET["accion"] == 111 and in_array(111, $privilegios) == true and !$_POST["buscar"]){
	...
	if ($_GET["accion"] == 112 and in_array(112, $privilegios) == true and !$_POST["buscar"]){
