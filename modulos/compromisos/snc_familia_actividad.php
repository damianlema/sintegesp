...
if ($_GET["accion"] == 114 || $_GET["accion"] == 115){   // si entra para modificar o eliminar busca el registro para cargar los datos en el formulario
...
<body>
	<br>
	<h4 align=center>Familias</h4>
	<h2 class="sqlmVersion"></h2>
	<br>
<form id="frmsnc_familia_actividad" action="principal.php?modulos=3&accion=<?=$_GET["accion"]?>" method="POST" enctype="multipart/form-data">	
...
<tr>
			  <td align='right' class='viewPropTitle'>Segmentos:</td>
			  <td class=''>
              <?php
              $sql = mysql_query("select * from snc_actividades");
			  ?>
              <select name="actividad" id="actividad" <? if($_GET["accion"] == 114){echo " disabled";}?>>
              <?php
              while($bus = mysql_fetch_array($sql)){
			  ?>
              <option <?php if(isset($_GET["acti"]) and $_GET["acti"] == $bus["idsnc_actividades"]){ echo " selected='selected'";}?> value="<?php echo $bus["idsnc_actividades"]?>"><?php echo $bus["sigla"]." ".$bus["descripcion"]?></option>
			  <?php
			  }
			  ?>
              </select>
&nbsp;<a href='principal.php?modulo=3&accion=80' class='Browse'><img src="imagenes/nuevo.png" border="0" title="Nuevo SNC Familia Actividad"></a>
          	...
          	<tr>
                ...
                <td>
                	<select name="ordenarPor" id="ordenarPor">
                    	 <option value="snc_actividades.descripcion">Segmentos</option>
                         <option value="snc_familia_actividad.descripcion">Familias</option>
                         <option value="snc_familia_actividad.codigo">C&oacute;digo</option>
                    </select>               
                </td>
            </tr>
            <tr>
          ...
          <tr>
				...
				<td class=''><input type="text" name="codigo" maxlength="2" size="4" id="codigo" <?php echo 'value="'.$regsnc_familia_actividad['codigo'].'"'; if($_GET["accion"] == 114){echo " disabled";}?> onKeyUp="validarVacios('codigo', this.value, 'frmsnc_familia_actividad')" onBlur="validarVacios('codigo', this.value, 'frmsnc_familia_actividad')" autocomplete="OFF" style="padding:0px 20px 0px 0px;"></td>
		  </tr>
			<tr>
				<td align='right' class='viewPropTitle'>Descripci&oacute;n:</td>
				<td class=''><input type="text" name="descripcion" maxlength="255" size="130" id="descripcion" <?php echo 'value="'.$regsnc_familia_actividad['descripcion'].'"';?> onKeyUp="validarVacios('descripcion', this.value, 'frmsnc_familia_actividad')" onBlur="validarVacios('descripcion', this.value, 'frmsnc_familia_actividad')" autocomplete="OFF" style="padding:0px 20px 0px 0px;"></td>
			</tr>
...
<table align=center cellpadding=2 cellspacing=0>
			<tr><td>
			<?php
					if($_GET["accion"] != 114 and $_GET["accion"] != 115 and in_array(113, $privilegios) == true){
						echo "<input align=center class='button' name='ingresar' type='submit' value='Ingresar'>";
					}
				
					if($_GET["accion"] == 114 and in_array($_GET["accion"], $privilegios) == true){
						echo "<input align=center class='button' name='modificar' type='submit' value='Modificar'>";
					}
				
					if($_GET["accion"] == 115 and in_array($_GET["accion"], $privilegios) == true){
						...
						<table class="Browse" align=center cellpadding="0" cellspacing="0" width="100%">
							<thead>
								<tr>
                                    ...
                                    <td align="center" class="Browse">Familias</td>
						  ...
						  <tr bgcolor='#e7dfce' onMouseOver="setRowColor(this, 0, 'over', '#e7dfce', '#EAFFEA', '#FFFFAA')" onMouseOut="setRowColor(this, 0, 'out', '#e7dfce', '#EAFFEA', '#FFFFAA')">
								...
								$bus = mysql_fetch_array($sql);
									...
									if(in_array(114, $privilegios) == true){
										echo "<td align='center' class='Browse' width='3%'><a href='principal.php?modulo=3&accion=114&c=$c&acti=".$bus["idsnc_actividades"]."' class='Browse'><img src='imagenes/modificar.png' border='0' alt='Modificar' title='Modificar'></a></td>";
									}
									if(in_array(115, $privilegios) == true){
										echo "<td align='center' class='Browse' width='3%'><a href='principal.php?modulo=3&accion=115&c=$c' class='Browse'><img src='imagenes/delete.png' border='0' alt='Borrar' title='Borrar'></a></td>";	
									}
									echo "</tr>";
									}
							}?>
...
if($_POST){
	...
	if($_GET["accion"] == 113 and in_array(113,$privilegios) == true){
	
	if (mysql_num_rows($busca_existe_registro)>0){
		mensaje("Disculpe el registro que ingreso ya existe");
		redirecciona("principal.php?modulo=3&accion=80");
	}else{
	...
	if ($_GET["accion"]==114 and in_array(114, $privilegios) == true and !$_POST["buscar"]){
	...
	if ($_GET["accion"] == 115 and in_array(115, $privilegios) == true and !$_POST["buscar"]){
