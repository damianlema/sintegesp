...
if($ejecutar == "actualizarDatos"){
	...
	$sql_consultar_descuento = mysql_query("select * from requisicion where idrequisicion = '".$id_requisicion."'");
...
/*	if($sub_total != $sub_total_viejo){
	...
	*/
	...
	$sql_actualizar_datos = mysql_query("update requisicion set justificacion = '".$justificacion."',
																			exento = '".$exento."',
																			sub_total = '".$sub_total."',
																			impuesto = '".$impuesto."',
																			total = '".$total."',
																			descuento = '".$descuento."'
																			where idrequisicion = '".$id_requisicion."'")or die("Error en el Primer Query: ".mysql_error());
...
if($ejecutar == "buscarOrdenes"){
    ...
    <?
    $sql_requisicion = mysql_query("select * from requisicion where numero_requisicion like '%".$numero_requisicion."%' and numero_requisicion != '' order by codigo_referencia ASC");
    $i = 0;
    while($bus_requisicion = mysql_fetch_array($sql_requisicion)){
			?>
			<tr bgcolor='#e7dfce' onMouseOver="setRowColor(this, 0, 'over', '#e7dfce', '#EAFFEA', '#FFFFAA')" onMouseOut="setRowColor(this, 0, 'out', '#e7dfce', '#EAFFEA', '#FFFFAA')">
            <?
            $sql_beneficiario = mysql_query("select * from beneficiarios where idbeneficiarios = '".$bus_requisicion["idbeneficiarios"]."'");
			$bus_beneficiario = mysql_fetch_array($sql_beneficiario);
			?>
            <td align="center" class='Browse'><?=$i+1?></td>
            <td align="left" class='Browse'><?=$bus_requisicion["numero_requisicion"]?></td>
            <td align="left" class='Browse'><?=$bus_beneficiario["nombre"]?></td>
            <td align="left" class='Browse'><?=$bus_requisicion["justificacion"]?>
            <input type="hidden" id="justifica" value='<?=$bus_requisicion["justificacion"]?>'></td>
            <td align="right" class='Browse'><?=$bus_requisicion["exento"]?></td>
			<td align="right" class='Browse'><?=$bus_requisicion["sub_total"]?></td>
            <td align="right" class='Browse'><?=$bus_requisicion["impuesto"]?></td>
            <td align="right" class='Browse'><?=$bus_requisicion["descuento"]?></td>
            <td align="right" class='Browse'><?=$bus_requisicion["total"]?></td>
            <td align="center" class='Browse'>
            <img src="imagenes/validar.png" 
                title="Actualizar Descuento" 
                style="cursor:pointer" 
                onclick="mostrarDatos('<?=$bus_requisicion["idrequisicion"]?>',
                						'<?=$bus_requisicion["numero_requisicion"]?>',
                                        '',
                                        '<?=$bus_requisicion["exento"]?>',
                                        '<?=$bus_requisicion["sub_total"]?>',
                                        '<?=$bus_requisicion["impuesto"]?>',
                                        '<?=$bus_requisicion["descuento"]?>',
                                        '<?=$bus_requisicion["total"]?>',
                                        '<?=number_format($bus_requisicion["exento"],2,",",".")?>',
                                        '<?=number_format($bus_requisicion["sub_total"],2,",",".")?>',
                                        '<?=number_format($bus_requisicion["impuesto"],2,",",".")?>',
                                        '<?=number_format($bus_requisicion["descuento"],2,",",".")?>',
                                        '<?=number_format($bus_requisicion["total"],2,",",".")?>')">
            </td>
      </tr>
        <?
		$i++;
...
if($ejecutar == "listarPartidas"){
	$sql_partidas =  mysql_query("select * from partidas_requisiciones where idrequisicion = '".$id_requisicion."'")or die(mysql_error());
?>
           ...
           <tr bgcolor='#e7dfce' onMouseOver="setRowColor(this, 0, 'over', '#e7dfce', '#EAFFEA', '#FFFFAA')" onMouseOut="setRowColor(this, 0, 'out', '#e7dfce', '#EAFFEA', '#FFFFAA')">
            ...
            <td width="6%" align='center' class='Browse'>
            	<img src="imagenes/refrescar.png" 
                    ...
                    onclick="actualizarPartidas('<?=$bus_partidas["idpartidas_requisiciones"]?>', document.getElementById('monto<?=$bus_partidas["idmaestro_presupuesto"]?>').value)"></td>
            <td width="6%" align='center' class='Browse'>
...
if($ejecutar == "actualizarPartidas"){
	$sql_partida = mysql_query("select * from partidas_requisiciones where idpartidas_requisiciones = '".$idpartida."'");
	$bus_partida = mysql_fetch_array($sql_partida);
	$sql_actualizar_partida = mysql_query("update partidas_requisiciones set monto = '".$monto_actualizar."'
																				where idpartidas_requisiciones = '".$idpartida."'")or die(mysql_error());
...
if($ejecutar == "volverMontosOriginales"){
	$sql_orden = mysql_query("select * from requisicion where idrequisicion = '".$id_requisicion."'")or die(mysql_error());
	...
	$sql_impuestos = mysql_query("select * from relacion_impuestos_requisiciones where idrequisicion = '".$id_requisicion."'");
		while($bus_impuestos = mysql_fetch_array($sql_impuestos)){
