...
switch($ejecutar){
	...
	$sql_consulta = mysql_query("select * from asociar_requisitos_actividad_comercial 
											where 
											idactividad_comercial = '".$idactividad_comercial."' 
											and idrequisitos = '".$idrequisitos."'");
	$num_consulta = mysql_num_rows($sql_consulta);
	if($num_consulta == 0){
	$sql_ingresar = mysql_query("insert into asociar_requisitos_actividad_comercial(idrequisitos, 
														idactividad_comercial 
														)VALUES('".$idrequisitos."', 
																			'".$idactividad_comercial."')")or die(mysql_error());
	}else{
		echo "existe";
	}
	break;
	case "listarRequisitos":
		$sql_consulta = mysql_query("select re.denominacion,
											arc.idasociar_requisitos_actividad_comercial
											from 
												requisitos re,
												asociar_requisitos_actividad_comercial arc
											where 
		...
		while($bus_consulta = mysql_fetch_array($sql_consulta)){
			...
			<tr bgcolor='#e7dfce' onMouseOver="setRowColor(this, 0, 'over', '#e7dfce', '#EAFFEA', '#FFFFAA')" onMouseOut="setRowColor(this, 0, 'out', '#e7dfce', '#EAFFEA', '#FFFFAA')">
              ...
              <td width="11%" align='center' class='Browse'><img src="imagenes/delete.png" style="cursor:pointer" onClick="eliminarRequisitos('<?=$bus_consulta["idasociar_requisitos_actividad_comercial"]?>', '<?=$idactividad_comercial?>')"></td>
          </tr>
			<?
		}
		?>
	...
	case "eliminarRequisitos":
		...
		//if($num_consultar == 0){	
			$sql_eliminar = mysql_query("delete from asociar_requisitos_actividad_comercial where idasociar_requisitos_actividad_comercial='".$idasociar_requisitos_actividad_comercial."'")or die(mysql_error());
		//}else{
		//	echo "usado";
		//}
