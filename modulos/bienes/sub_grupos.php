<script src="modulos/bienes/js/sub_grupos_ajax.js"></script>
    <br>
<h4 align=center>
Sub Grupos</h4>
...
<table width="60%" border="0" align="center">
  <tr>
    ...
    <td><label>
      ...
      <?
      $sql_consultar_grupos = mysql_query("select * from grupo_catalogo_bienes");
	  $i=0;
	  while($bus_consultar_grupos = mysql_fetch_array($sql_consultar_grupos)){
		...
		?>
		<option onclick="document.getElementById('codigoGrupo').innerHTML='<?=$bus_consultar_grupos["codigo"]?>-', document.getElementById('codigoHidden').value='<?=$bus_consultar_grupos["codigo"]?>-'" value="<?=$bus_consultar_grupos["idgrupo_catalogo_bienes"]?>">
			<?=$bus_consultar_grupos["denominacion"]?>
        </option>
		<?
	  }
	  ?>
...
<div id="listaGrupos">
<?
$sql_consulta = mysql_query("select * from subgrupo_catalogo_bienes");
							...
							<tr bgcolor='#e7dfce' onMouseOver="setRowColor(this, 0, 'over', '#e7dfce', '#EAFFEA', '#FFFFAA')" onMouseOut="setRowColor(this, 0, 'out', '#e7dfce', '#EAFFEA', '#FFFFAA')">
								...
								?>
                                	...
                                	<img src="imagenes/modificar.png" 
                                    	onclick="mostrarEditar('<?=$bus_consulta["idsubgrupo_catalogo_bienes"]?>',
                                        						'<?=$bus_consulta["idgrupo_catalogo_bienes"]?>',
                                        						'<?=$codigoHidden?>',
                                                                '<?=$codigo?>',
                                ...
                                <td width="6%" align="center" class='Browse'>
                                    ...
                                    onclick="mostrarEliminar('<?=$bus_consulta["idsubgrupo_catalogo_bienes"]?>',
                                    							'<?=$bus_consulta["idgrupo_catalogo_bienes"]?>',
                                        						'<?=$codigoHidden?>',
                                                                '<?=$codigo?>',
