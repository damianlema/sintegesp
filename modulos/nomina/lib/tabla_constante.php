...
if($ejecutar== "ingresar"){
	...
	if($codigo == "" || $descripcion=="" || $desde=="" || $hasta==""){
		...
		$str_sql = "insert into tabla_constantes(codigo,
												 descripcion,
												 desde,
												 hasta,
												 unidad,
...
<br />
        ...
        <?
        $sql_consulta = mysql_query("select * from rango_tabla_constantes where idtabla_constantes = '".$idtabla_constantes."' order by desde, hasta");
		$n = 0;
		...
		<tr bgcolor='#e7dfce' onMouseOver="setRowColor(this, 0, 'over', '#e7dfce', '#EAFFEA', '#FFFFAA')" onMouseOut="setRowColor(this, 0, 'out', '#e7dfce', '#EAFFEA', '#FFFFAA')">
            ...
            <td align="right" class='Browse'><?=number_format($bus_consulta["valor"],4,",",".")?></td>
...
if($ejecutar == "guardarRango"){
		...
		$sql_consulta = mysql_Query("select * from rango_tabla_constantes where (desde = '".$desdeRango."' or hasta = '".$hastaRango."') and idtabla_constantes = '".$idtabla_constantes."'");
		...
		if($desdeRango == "" || $hastaRango == "" || $valor == ""){
			...
			echo $accion;
				...
				$str_guardar_rango = "insert into rango_tabla_constantes (
																		  idtabla_constantes,
																		  desde,
																		  hasta,
																		  valor)values(
	...
	if($ejecutar == "eliminarRango"){
		$sql_eliminar = mysql_query("delete from rango_tabla_constantes where idrango_tabla_constantes = '".$idrango_tabla_constantes."'");
	}
	...
	if($ejecutar == "consultarTablaConstantes"){
		$sql_consultar = mysql_query("select * from tabla_constantes where idtabla_constantes = '".$idtabla_constantes."'");
	...
	if($codigo == "" || $descripcion=="" || $desde=="" || $hasta==""){
		...
		$str_sql = "update tabla_constantes set codigo = '".$codigo."',
	...
	if($ejecutar == "eliminarTablaConstantes"){
		$sql_eliminar = mysql_query("delete from tabla_constantes where idtabla_constantes = '".$idtabla_constantes."'");
		$sql_eliminar_rango = mysql_query("delete from rango_tabla_constantes where idtabla_constantes = '".$idtabla_constantes."'");
	...
	if($ejecutar == "modificarRango"){
		$sql_actualizar = mysql_query("update rango_tabla_constantes set desde = '".$desde."',
									  									hasta = '".$hasta."',
																		valor = '".$valor."'
