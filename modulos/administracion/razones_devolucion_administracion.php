...
if ($_GET["accion"] == 350 || $_GET["accion"] == 351){   // si entra para modificar o eliminar busca el registro para cargar los datos en el formulario
...
// end hiding from old browsers -->
	...
	<br>
	<form id="frmrazones_devolucion" action="principal.php?modulo=4&accion=<?=$_GET["accion"]?>" method="POST" enctype="multipart/form-data" onSubmit="return valida_envia()">	
...
<tr>
				...
				<td class=''><input type="text" name="descripcion" maxlength="255" size="45" id="descripcion" <?php echo 'value="'.$regrazones_devolucion['descripcion'].'"';?> onKeyUp="validarVacios('descripcion', this.value, 'frmrazones_devolucion')" onBlur="validarVacios('descripcion', this.value, 'frmrazones_devolucion')" autocomplete="OFF" style="padding:0px 20px 0px 0px;">
			    &nbsp;<a href='principal.php?modulo=2&accion=269' class='Browse'><img src="imagenes/nuevo.png" border="0" title="Nueva Raz&oacute;n de Devoluci&oacute;n"></a>
...
<table align=center cellpadding=2 cellspacing=0>
	...
	?>
			<?php
					if($_GET["accion"] != 271 and $_GET["accion"] != 272 and in_array(270, $privilegios) == true){
						echo "<input align=center class='button' name='ingresar' type='submit' value='Ingresar'>";
					}
				
					if($_GET["accion"] == 271 and in_array($_GET["accion"], $privilegios) == true){
						echo "<input align=center class='button' name='modificar' type='submit' value='Modificar'>";
					}
				
					if($_GET["accion"] == 272 and in_array($_GET["accion"], $privilegios) == true){
						...
						<table class="Browse" align=center cellpadding="0" cellspacing="0" width="50%">
							...
							if ($existen_registros==0){
								...
								<?php
									...
									if(in_array(271,$privilegios) == true){
										echo "<td align='center' class='Browse' width='3%'><a href='principal.php?modulo=2&accion=271&c=$c' class='Browse'><img src='imagenes/modificar.png' border='0' alt='Modificar' title='Modificar'></a></td>";
									}
									if(in_array(272,$privilegios) ==  true){
										echo "<td align='center' class='Browse' width='3%'><a href='principal.php?modulo=2&accion=272&c=$c' class='Browse'><img src='imagenes/delete.png' border='0' alt='Borrar' title='Borrar'></a></td>";	
									}
									echo "</tr>";
									}
							}?>
...
if($_POST){
	...
	if ($_GET["accion"] == 271 and in_array(271,$privilegios) == true and !$_POST["buscar"]){
	...
	if ($_GET["accion"] == 272 and in_array(272,$privilegios) == true and !$_POST["buscar"]){
