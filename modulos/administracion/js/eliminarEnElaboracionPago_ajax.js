...
function eliminarOrdenCompra(id_orden_pago, botonEliminar){
	var ajax=nuevoAjax();
	ajax.open("POST", "modulos/administracion/lib/eliminarEnElaboracionPagos_ajax.php?", true);	
	...
	ajax.onreadystatechange=function() { 
		...
		if (ajax.readyState==4){
				if(ajax.responseText == 'exito'){
					buscarOrdenes();
					mostrarMensajes("exito", "Ordenes Eliminadas con Exito");
				}else{
	...
	ajax.send("id_orden_pago="+id_orden_pago+"&ejecutar=eliminarOrdenCompra");	
}
