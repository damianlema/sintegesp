...
if($ejecutar == "duplicarOrden"){
	...
	}else{ // SI NO ES NI SIN AFECTACION NI PAGO DIRECTO SINO QUE ES CUALQUIERA DE LOS TIPO CONTRA CUALQUIER COMPROMISO
		$sql_insert_orden = mysql_query("insert into orden_pago(tipo,
																		fecha_elaboracion,
																		proceso,
																		numero_documento,
...
if($ejecutar == "ingresarOrdenCreada"){
	...
	if($num_validar_relacion == 0){
		...
		if ($num_validar_numero_filas < $bus_numero_filas["nro_linea_comprobante"]){ //valido que el numero de compromisos seleccionado sea menor a la
		...
		}else{ //PROCESOS SI SE TRATA DE UN ANTICIPO
		...
		}else{ // si ya completo las filas de un comprobante evito que siga ingresando compromisos
			echo "filasagotadas";
		}
		$sql_orden_pago = mysql_query("select * from orden_pago where idorden_pago = ".$id_orden_pago."");
		$bus_orden_pago = mysql_fetch_array($sql_orden_pago);
