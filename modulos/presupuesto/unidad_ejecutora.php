...
if ($_GET["accion"] == 165 and isset($_GET["c"])){   // si entra para modificar o eliminar busca el registro para cargar los datos en el formulario
...
if ($_GET["accion"] == 166 and isset($_GET["c"])){   // si entra para eliminar busca el registro para cargar los datos en el formulario
...
function abreVentana(){
	...
	miPopup=window.open("lib/lista_cargos.php?m="+m,"cargos","width=600,height=400,scrollbars=yes")
	miPopup.focus()
}
	...
	<input type="hidden" id="codigo2" name="codigo2" maxlength="4" size="4" <?php if (isset($_POST["codigo"])){ echo 'value="'.$_POST["codigo"].'"';} else {echo 'value="'.$regunidad_ejecutora["codigo"].'"';}?>>
		<table align=center cellpadding=2 cellspacing=0 width="60%">
			<tr>
				...
				<td class=''><input type="text" id="codigo" name="codigo" maxlength="4" size="4" onKeyPress="javascript:return solonumeros(event)" <?php if (isset($_POST["codigo"])){ echo 'value="'.$_POST["codigo"].'"';} else {echo 'value="'.$regunidad_ejecutora["codigo"].'"';} if ($_GET["accion"]==166) echo "disabled";?>>
          ...
          <iframe name="pdf" id="pdf" style="display:none" height="500" width="500"></iframe>          
			...
			<tr>
				...
				<td class=''><input type="text" name="denominacion" maxlength="255" size="65" id="denominacion" <?php if (isset($_POST["codigo"])){ echo 'value="'.$_POST["denominacion"].'"';} if ($_GET["accion"]==165 || $_GET["accion"]==166) {echo 'value="'.$regunidad_ejecutora['denominacion'].'"';} if ($_GET["accion"]==166) echo "disabled";?>></td>
			</tr>
			<tr>
				<td align='right' class='viewPropTitle'>Responsable:</td>
				<td class=''><input type="text" name="responsable" maxlength="255" size="65" id="responsable" <?php if (isset($_POST["codigo"])){ echo 'value="'.$_POST["responsable"].'"';} if ($_GET["accion"]==165 || $_GET["accion"]==166) {echo 'value="'.$regunidad_ejecutora['responsable'].'"';} if ($_GET["accion"]==166) echo "disabled";?>></td>
			</tr>
			<tr>
				<td align='right' class='viewPropTitle'>Cargo:</td>
				<td class=''><input type="text" name="cargos" id="cargos" maxlength="95" size="45"  <?php if (!$_POST["idcargo"]){ echo ' value="'.$regunidad_ejecutora["cargo"].'"';}else{ echo ' value="'.$regcargos["denominacion"].'"';} if ($_GET["accion"]==166) echo "disabled";?>>
