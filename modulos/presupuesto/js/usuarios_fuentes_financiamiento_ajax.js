...
function pasarUsuario(){
	...
	var idfuente=document.getElementById("codigo").value;
	var ajax=nuevoAjax();
	if (cedula_usuario_seleccionado != ""){
		ajax.open("POST", "modulos/presupuesto/lib/usuarios_fuentes_financiamiento_ajax.php", true);
	...
	ajax.send("cedula_usuario_seleccionado="+cedula_usuario_seleccionado+"&idfuente="+idfuente+"&ejecutar=pasarUsuario");
}
...
function regresarUsuario(){
	...
	ajax.send("cedula_usuario_seleccionado="+cedula_usuario_seleccionado+"&idfuente="+idfuente+"&ejecutar=regresarUsuario");
}