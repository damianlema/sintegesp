<script src="modulos/almacen/js/detalle_materias_almacen_ajax.js"></script>
    <br>
<h4 align=center>
Detalles</h4>
...
<table width="60%" border="0" align="center">
  <tr>
    ...
    <td><label>
      ...
      <?
      $sql_consultar_secciones= mysql_query("select * from seccion_materias_almacen");
	  $i=0;
	  while($bus_consultar_secciones = mysql_fetch_array($sql_consultar_secciones)){
		...
		?>
		<option onClick="document.getElementById('codigoDetalle').innerHTML='<?=$bus_consultar_secciones["codigo"]?>-', document.getElementById('codigoHidden').value='<?=$bus_consultar_secciones["codigo"]?>-'" value="<?=$bus_consultar_secciones["idseccion_materias_almacen"]?>">
			<?=$bus_consultar_secciones["codigo"]." - ".$bus_consultar_secciones["denominacion"]?>
        </option>
		<?
	  }
	  ?>
