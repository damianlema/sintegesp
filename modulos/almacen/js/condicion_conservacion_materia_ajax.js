...
function ingresarCondicion(){
	...
	if(denominacion == ''){
		...
		var ajax=nuevoAjax();
		ajax.open("POST", "modulos/almacen/lib/condicion_conservacion_materia_ajax.php", true);
		...
		ajax.onreadystatechange=function() { 
			...
			if (ajax.readyState==4){
				...
				if(externo == "si"){
					opener.cargarSelect('condicion_conservacion_materia', 'celda_condicion_conservacion', denominacion);
...
function modificarCondicion(){
	var denominacion = document.getElementById('denominacion').value;
	var idcondicion_conservacion = document.getElementById('idcondicion_conservacion').value;
	if(denominacion == ''){
		mostrarMensajes("error", "Disculpe debe escribir la Denominacion");
	}else{
		if (document.getElementById('preseleccion').checked == true){
		...
		ajax.send("idcondicion_conservacion="+idcondicion_conservacion+"&denominacion="+denominacion+"&preseleccion="+preseleccion+"&ejecutar=modificarCondicion");
	}
}
...
function eliminarCondicion(){
	var idcondicion_conservacion = document.getElementById('idcondicion_conservacion').value;
		...
		ajax.send("idcondicion_conservacion="+idcondicion_conservacion+"&ejecutar=eliminarCondicion");
}



function seleccionarModificar(idcondicion_conservacion, denominacion, seleccionada){
	...
	document.getElementById('idcondicion_conservacion').value = idcondicion_conservacion;
	document.getElementById('denominacion').disabled = false;
...
function seleccionarEliminar(idcondicion_conservacion, denominacion, seleccionada){
	document.getElementById('denominacion').value = denominacion;
	if (seleccionada == 1){
		document.getElementById('preseleccion').checked = true;
	}else{
