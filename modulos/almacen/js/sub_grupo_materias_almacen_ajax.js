...
function ingresarSubGrupos(grupo, codigo_hidden, codigo, denominacion){
	if(codigo == "" || denominacion == ""){
		...
		var ajax=nuevoAjax();
		ajax.open("POST", "modulos/almacen/lib/sub_grupo_materias_almacen_ajax.php", true);	
		...
		ajax.onreadystatechange=function() { 
			...
			if (ajax.readyState==4){
					...
					if(ajax.responseText == "existe"){
						mostrarMensajes("error", "Disculpe el Sub-Grupo que intenta ingresar ya existe");
					}
					if(ajax.responseText == "fallo"){
						mostrarMensajes("error", "Disculpe los datos no fueron ingresados con exito, por favor intente de nuevo");
					}
