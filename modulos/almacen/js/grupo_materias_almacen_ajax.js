...
function ingresarGrupos(codigo, denominacion){
	if(codigo == "" || denominacion == ""){
		...
		var ajax=nuevoAjax();
		ajax.open("POST", "modulos/almacen/lib/grupo_materias_almacen_ajax.php", true);	
		...
		ajax.onreadystatechange=function() { 
			...
			if (ajax.readyState==4){
				...
				if(ajax.responseText == "fallo"){
					mostrarMensajes("error", "Disculpe el Grupo no se pudo ingresar con Exito, por favor intente de nuevo");
				}	
					consultarGrupos();
