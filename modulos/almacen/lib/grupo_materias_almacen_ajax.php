...
if($ejecutar == "ingresarGrupos"){
	...
	$sql_consulta = mysql_query("select * from grupo_materias_almacen WHERE
								 codigo = '".$codigo."'");
	$bus_existe = mysql_num_rows($sql_consulta);
	if ($bus_existe == 0){
	$sql_ingresar_grupos = mysql_query("insert into grupo_materias_almacen(codigo,
																			denominacion,
																			status,
																			usuario,
																			fechayhora)values(
...
$sql_consulta = mysql_query("select * from grupo_materias_almacen");
?>
...
<thead>
							...
							<tr bgcolor='#e7dfce' onMouseOver="setRowColor(this, 0, 'over', '#e7dfce', '#EAFFEA', '#FFFFAA')" onMouseOut="setRowColor(this, 0, 'out', '#e7dfce', '#EAFFEA', '#FFFFAA')">
                                ...
                                <td width="6%" align="center" class='Browse'>
                                	<img src="imagenes/modificar.png" 
                                    	onclick="mostrarEditar('<?=$bus_consulta["idgrupo_materias_almacen"]?>',
                                        						'<?=$bus_consulta["codigo"]?>',
                          ...
                          <td width="6%" align="center" class='Browse'>
                                    ...
                                    onclick="mostrarEliminar('<?=$bus_consulta["idgrupo_materias_almacen"]?>',
                                        						'<?=$bus_consulta["codigo"]?>',
...
if($ejecutar == "modificarGrupos"){
	$sql_editar_grupo = mysql_query("update grupo_materias_almacen set codigo = '".$codigo."',
																	denominacion = '".$denominacion."'
																	where idgrupo_materias_almacen = '".$id_grupo."'");
	if($sql_editar_grupo){
		echo "exito";
	}else{
		echo "fallo";
...
if($ejecutar == "eliminarGrupos"){
	$sql_eliminar_grupo = mysql_query("delete from grupo_materias_almacen where idgrupo_materias_almacen = '".$id_grupo."'");
