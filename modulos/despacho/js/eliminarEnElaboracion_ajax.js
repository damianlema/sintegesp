...
function eliminarOrdenCompra(id_orden_compra, botonEliminar){
	var ajax=nuevoAjax();
	ajax.open("POST", "modulos/despacho/lib/eliminarEnElaboracion_ajax.php", true);
	...
	ajax.onreadystatechange=function() { 
		...
		if (ajax.readyState==4){
				if(ajax.responseText == 'exito'){
					mostrarMensajes("exito", "Orden Eliminada con exito");
					buscarOrdenes();
				}else{
...
function buscarOrdenes(){
	...
	var ajax=nuevoAjax();
	ajax.open("POST", "modulos/despacho/lib/eliminarEnElaboracion_ajax.phps", true);
	ajax.setRequestHeader('Content-Type', 'application/x-www-form-urlencoded; charset=ISO-8859-1');
	ajax.onreadystatechange=function() { 
		if(ajax.readyState == 1){
	...
	ajax.send("campoBuscar="+campoBuscar+"&ejecutar=buscarOrdene");
}