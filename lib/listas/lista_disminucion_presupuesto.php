...
/**
*
*	 "lista_disminucion_presupuesto.php" Listado de Dsiminuciones Presupuestarias
*	Version: 1.0.1
...
if($_GET["eliminar"]){
	$sql_eliminar = mysql_query("delete from disminucion_presupuesto where iddisminucion_presupuesto = '".$_GET["eliminar"]."'");
	$sql_eliminar = mysql_query("delete from partidas_disminucion_presupuesto where iddisminucion_presupuesto = '".$_GET["eliminar"]."'");
}

$registros_grilla=mysql_query("select * from disminucion_presupuesto where status='a' order by nro_solicitud",$conexion_db); 

if (isset($_POST["buscar"])){
	$texto_buscar=$_POST["textoabuscar"];
	$campo_busqueda=$_POST["tipobusqueda"];
	$sql="select * from disminucion_presupuesto where status='a'";
...
function ponDisminucion(idDebito){
	m=document.buscar.modoactual.value
	opener.document.forms[0].iddisminucionpresupuesto_emergente.value=idDebito
	opener.document.forms[0].iddisminucion_presupuesto.value=idDebito
	...
	<body>
	<br>
	<h4 align=center>Listado de Disminuciones Presupuestarias</h4>
	<h2 class="sqlmVersion"></h2>
	<br>
	
	<?php //echo $m;?>
	
	<form name="buscar" action="lista_disminucion_presupuesto.php" method="POST">
	...
	<div align="center">
			<table class="Main" cellpadding="0" cellspacing="0" width="95%">
				<tr>
					<td>
						<form name="grilla" action="lista_disminucion_presupuesto.php" method="POST">
						<table class="Browse" align=center cellpadding="0" cellspacing="0" width="95%">
							...
							if ($existen_registros==0){
								while($llenar_grilla= mysql_fetch_array($registros_grilla)) 
									{ 
									$c=$llenar_grilla["iddisminucion_presupuesto"];
									?>
										<tr bgcolor='#e7dfce' onMouseOver="setRowColor(this, 0, 'over', '#e7dfce', '#EAFFEA', '#FFFFAA')" onMouseOut="setRowColor(this, 0, 'out', '#e7dfce', '#EAFFEA', '#FFFFAA')" style="cursor:pointer" onClick="ponDisminucion("<?=$c?>")">
								<?php
									...
									echo "<td align='left' class='Browse'>".$llenar_grilla["justificacion"]."</td>";
									echo "<td align='left' class='Browse'>".$llenar_grilla["estado"]."</td>";
									echo "<td align='center' class='Browse' width='7%'> <a href='#' onclick='ponDisminucion(".$c.")'><img src='../../imagenes/validar.png'></a></td>";
									if ($llenar_grilla["estado"] == 'Anulado'){ ?>
										<td align='center' class='Browse' width='7%'>
                                    	<a href="?eliminar=<?=$llenar_grilla["iddisminucion_presupuesto"]?>">
