...
if($_GET["eliminar"]){
	$sql_eliminar = mysql_query("delete from traslados_presupuestarios where idtraslados_presupuestarios = '".$_GET["eliminar"]."'");
	$sql_eliminar = mysql_query("delete from partidas_cedentes_traslado where idtraslados_presupuestarios = '".$_GET["eliminar"]."'");
	$sql_eliminar = mysql_query("delete from partidas_receptoras_traslado where idtraslados_presupuestarios = '".$_GET["eliminar"]."'");
}
$registros_grilla=mysql_query("select * from traslados_presupuestarios where status='a' order by nro_solicitud",$conexion_db); 

if (isset($_POST["buscar"])){
	$texto_buscar=$_POST["textoabuscar"];
	$campo_busqueda=$_POST["tipobusqueda"];
	$sql="select * from traslados_presupuestarios where status='a'";
...
function ponTraslado(idTraslado){
	...
	<body>
	<br>
	<h4 align=center>Listado de Traslados Presupuestarios</h4>
	<h2 class="sqlmVersion"></h2>
	<br>
	
	<?php //echo $m;?>
	
	<form name="buscar" action="lista_traslados_presupuestarios.php" method="POST">
						...
						<table class="Browse" align=center cellpadding="0" cellspacing="0" width="95%">
							...
							if ($existen_registros==0){
								while($llenar_grilla= mysql_fetch_array($registros_grilla)) 
									{ 
									$c=$llenar_grilla["idtraslados_presupuestarios"];
									?>
										<tr bgcolor='#e7dfce' onMouseOver="setRowColor(this, 0, 'over', '#e7dfce', '#EAFFEA', '#FFFFAA')" onMouseOut="setRowColor(this, 0, 'out', '#e7dfce', '#EAFFEA', '#FFFFAA')" style="cursor:pointer" onClick="ponTraslado('<?=$c?>')">
								<?php
									...
									echo "<td align='center' class='Browse' width='7%'> <a href='#' onclick='ponTraslado('".$c."')'><img src='../../imagenes/validar.png'></a></td>";
									if ($llenar_grilla["estado"] == 'Anulado'){ ?>
										<td align='center' class='Browse' width='7%'>
                                    	<a href="?eliminar=<?=$llenar_grilla["idtraslados_presupuestarios"]?>">
