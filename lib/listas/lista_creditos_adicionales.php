...
/**
*
*	 "lista_creditos_adicionales.php" Listado de Creditos Adicionales
*	Version: 1.0.1
...
if($_GET["eliminar"]){
	$sql_eliminar = mysql_query("delete from creditos_adicionales where idcreditos_adicionales = '".$_GET["eliminar"]."'");
	$sql_eliminar = mysql_query("delete from partidas_credito_adicional where idcreditos_adicionales = '".$_GET["eliminar"]."'");
}

$registros_grilla=mysql_query("select * from creditos_adicionales where status='a' order by nro_solicitud",$conexion_db); 

if (isset($_POST["buscar"])){
	$texto_buscar=$_POST["textoabuscar"];
	$campo_busqueda=$_POST["tipobusqueda"];
	$sql="select * from creditos_adicionales where status='a'";
...
function ponCredito(idCredito){
	m=document.buscar.modoactual.value
	opener.document.forms[0].idcreditoadicional_emergente.value=idCredito
	opener.document.forms[0].idcreditos_adicionales.value=idCredito
	...
	<body>
	<br>
	<h4 align=center>Listado de Cr&eacute;ditos Adicionales</h4>
	<h2 class="sqlmVersion"></h2>
	<br>
	
	<?php //echo $m;?>
	
	<form name="buscar" action="lista_creditos_adicionales.php" method="POST">
						...
						<table class="Browse" align=center cellpadding="0" cellspacing="0" width="95%">
							...
							if ($existen_registros==0){
								while($llenar_grilla= mysql_fetch_array($registros_grilla)) 
									{ 
									$c=$llenar_grilla["idcreditos_adicionales"];
									?>
										<tr bgcolor='#e7dfce' onMouseOver="setRowColor(this, 0, 'over', '#e7dfce', '#EAFFEA', '#FFFFAA')" onMouseOut="setRowColor(this, 0, 'out', '#e7dfce', '#EAFFEA', '#FFFFAA')" style="cursor:pointer" onClick="ponCredito('<?=$c?>')">
								<?php
									...
									echo "<td align='center' class='Browse' width='7%'> <a href='#' onclick='ponCredito(".$c.")'><img src='../../imagenes/validar.png'></a></td>";
									if ($llenar_grilla["estado"] == 'Anulado'){ ?>
										<td align='center' class='Browse' width='7%'>
                                    	<a href="?eliminar=<?=$llenar_grilla["idcreditos_adicionales"]?>">
