...
function trasladarCompromiso(idorden_compra_trasladar){
	var anio_compromiso = document.getElementById('anio').value;
	var anio_receptor = document.getElementById('anio_receptor').value;
	...
	if (fuente_financiamiento == 0){
		alert("Debe seleccionar una fuente de financiamiento a la cual se va a trasladar el compromiso");
	}else{
		if (tipo_presupuesto == 0){
			alert("Debe seleccionar el tipo de presupuesto al cual se va a trasladar el compromiso");
		}else{
			if (categoria_programatica == 0){
				alert("Debe seleccionar la categoria programatica a la cual se va a trasladar el compromiso");
			}else{
				var ajax=nuevoAjax();
				ajax.open("POST", "lib/traslada_compromiso/lib/traslada_compromiso_ajax.php", true);
				...
				ajax.send("id_orden_compra="+idorden_compra_trasladar+"&anio_receptor="+anio_receptor+"&fuente_financiamiento="+fuente_financiamiento+"&tipo_presupuesto="+tipo_presupuesto+"&categoria_programatica="+categoria_programatica+"&anio_compromiso="+anio_compromiso+"&ejecutar=trasladarCompromiso");
			}
		}
	}
}
